...
float4			main					(PixelShaderInput input) : SV_TARGET {
	...
	materialColor	= markCenter
		? saturate(float4(materialColor.r + max(0, Diffuse.b * diffuseFactor * 2) + max(0, Diffuse.g * randFactor), materialColor.g + max(0, Diffuse.r * diffuseFactor * 2) + max(0, Diffuse.b * randFactor), materialColor.b + max(0, Diffuse.g * diffuseFactor * 2 - .15f) + max(0, Diffuse.r * randFactor), max(Diffuse.a * diffuseFactor * 4, .25f)))
