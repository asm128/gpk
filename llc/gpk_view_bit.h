...
namespace gpk
{
	...
	struct view_bit_iterator {
		...
		view_bit_iterator&			operator++		()				{ ++Offset; if(Offset >= ELEMENT_BITS)	{ ++Element; Offset = 0;				gthrow_if(Element >= (End	+ 1), "Out of range: %u. End: %u.", Element, End); } return *this; }
		view_bit_iterator&			operator--		()				{ --Offset; if(Offset < 0)				{ --Element; Offset = ELEMENT_BITS - 1; gthrow_if(Element <  (Begin	- 1), "Out of range: %u. End: %u.", Element, Begin); } return *this; }
	...
	public:
		...
		stacxpr	uint16_t			ELEMENT_BITS		= sizeof(T) * 8;
		...
		bool						operator[]			(uint32_t index)	const	{
			gthrow_if(0 == Data, "Uninitialized array pointer. Invalid index: %u.", index);
			gthrow_if(index >= Count, "Invalid index: %u.", index);
			const uint32_t					offseT				= index / ELEMENT_BITS;
			const uint32_t					offsetLocal			= index % ELEMENT_BITS;
			const T							& selectedElement	= Data[offseT];
