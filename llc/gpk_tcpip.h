...
#define GPK_IPV4_EXPAND_PTR_IP(addr) &addr.IP[0]	\
	...
	static inline	::gpk::error_t									tcpipAddress						(const ::gpk::view_const_char & hostName, uint16_t portRequested, uint32_t adapterIndex, TRANSPORT_PROTOCOL mode, uint8_t* a1, uint8_t* a2, uint8_t* a3, uint8_t* a4, uint16_t* port = 0) { return ::gpk::tcpipAddress(::gpk::toString(hostName).begin(), portRequested, adapterIndex, mode, a1, a2, a3, a4, port); }
