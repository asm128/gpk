...
::gpk::error_t									evaluateNode							(const ::gpk::SExpressionNode* nodeExpression, const ::gpk::view_array<::gpk::view_const_string>& viewsExpression, const ::gpk::SJSONNode& input, const ::gpk::view_array<::gpk::view_const_string>& viewsJSON, ::gpk::view_const_string& output)						{
	if(0 == nodeExpression->Children.size()) {
		if(nodeExpression->Object->Type == ::gpk::EXPRESSION_READER_TYPE_KEY) {
			...
			int32_t												jsonIndex								= ::gpk::jsonObjectValueGet(input, viewsJSON, viewsExpression[nodeExpression->ObjectIndex]);
		...
		for(uint32_t iChild = 0; iChild < nodeExpression->Children.size(); ++iChild) {
			gpk_necall(::evaluateNode(nodeExpression->Children[iChild], viewsExpression, input, viewsJSON, output), "Failed to evaluate node at index %i.", nodeExpression->Children[iChild]->ObjectIndex);
		}
	return 0;
}

::gpk::error_t									jsonStringFormatResolve					(const ::gpk::view_const_string & expression, const ::gpk::SJSONNode& input, const ::gpk::view_array<::gpk::view_const_string>& viewsJSON, ::gpk::view_const_string& output)		{
