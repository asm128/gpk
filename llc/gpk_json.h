...
#pragma pack(pop)
	...
	static inline			::gpk::error_t												jsonObjectValueGet		(const ::gpk::SJSONReader& reader, uint32_t iNode, const ::gpk::view_const_char& key)	noexcept	{ ree_if(iNode >= reader.Tree.size(), "Invalid node index: %i. Total nodes: %u", iNode, reader.Tree.size()); return ::gpk::jsonObjectValueGet(*reader.Tree[iNode], reader.View, {key.begin(), key.size()}); }
	...
	static inline			::gpk::error_t												jsonObjectKeyList		(const ::gpk::SJSONReader& reader, uint32_t iNode, const ::gpk::view_array<::gpk::view_const_string>& views, ::gpk::array_obj<::gpk::view_const_string> & keys)											{ ree_if(iNode >= reader.Tree.size(), "Invalid node index: %i. Total nodes: %u", iNode, reader.Tree.size()); return jsonObjectKeyList(*reader.Tree[iNode], views, keys);				}
