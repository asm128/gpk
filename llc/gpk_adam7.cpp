...
::gpk::error_t								gpk::adam7Sizes			(::gpk::view<::gpk::n2<uint32_t>> & imageSizes, const ::gpk::n2<uint32_t> & imageSize)	{
	imageSizes	[0]								= // 1
		{ (imageSize.x <= 4)	? 0U : imageSize.x / 8 + one_if(imageSize.x % 8)
	...
	imageSizes	[3]								= // 4
		{ (imageSize.x <= 2)	? 0 : (uint32_t)::gpk::max(((int32_t)imageSize.x - 2) / 4, (int32_t)0) + one_if(((int32_t)imageSize.x - 2) % 4)
		, (imageSize.y <= 4)	? 0 : imageSize.y / 4 + one_if(imageSize.y % 4)
		};
	imageSizes	[4]								= // 5
		{ (imageSize.x <  2)	? 0 : (imageSize.x) / 2 + one_if(imageSize.x % 2)
		, (imageSize.y <= 2)	? 0 : (uint32_t)::gpk::max(((int32_t)imageSize.y - 2) / 4, (int32_t)0) + one_if(((int32_t)imageSize.y - 2) % 4)
		};
	imageSizes	[5]								= // 6
		{ (imageSize.x < 2)		? 0 : (uint32_t)::gpk::max(((int32_t)imageSize.x - 1) / 2, (int32_t)0) + one_if(((int32_t)imageSize.x - 1) % 2)
