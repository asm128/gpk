...
int										main							(int argc, char ** argv)						{
	...
	::gpk::view_const_string					nameFileSrc						= {argv[0], -1};
	if(2 < argc)
		namePathDst								= ::gpk::toString({argv[2], -1});
	else if(2 == argc) {
		namePathDst								= ::gpk::toString({argv[1], -1});
	...
	gpk_necall(::gpk::folderUnpackToDisk({namePathDst.begin(), namePathDst.size()}, nameFileSrc), "Failed to unpack file '%s' to folder '%s'.", ::gpk::toString(nameFileSrc), namePathDst);
