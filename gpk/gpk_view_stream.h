...
namespace gpk
{
	...
	struct view_stream : public view_array<_tElement> {
		...
		template<typename _tPOD>					inline	::gpk::error_t					write_pod							(const _tPOD& towrite)					{ memcpy(&Data[CursorPosition], &towrite, sizeof(_tPOD));																					CursorPosition += sizeof(_tPOD);	return sizeof(_tPOD); }
		template<typename _tOBJ>					inline	::gpk::error_t					write_obj							(const _tOBJ& towrite)					{ *(_tOBJ*)&Data[CursorPosition]	= towrite;																								CursorPosition += sizeof(_tOBJ);	return sizeof(_tOBJ); }
