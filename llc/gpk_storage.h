...
namespace gpk
{
	...
	static inline	::gpk::error_t									pathList				(const ::gpk::view_const_char& pathToList, ::gpk::array_obj<::gpk::array_pod<char_t>>& output) {
		...
		return 0;
					...
					::gpk::error_t									fileSplit				(const ::gpk::view_const_char& fileNameSrc, const uint32_t partSize);	// Splits a file into file.split.## parts.
					int64_t											fileSize				(const ::gpk::view_const_char& fileNameSrc);
					::gpk::error_t									pathCreate				(const ::gpk::view_const_char& folderName, const char separator = '/');	// Recursive
	...
	// this function was ceated in order to work around the problem of the JSON system returning pointers to the original string, without having the opportunity of processing escaped path slashes.
					::gpk::error_t									pathNameCompose			(::gpk::view_const_char path, ::gpk::view_const_char fileName, ::gpk::array_pod<char_t> & out_composed);
					::gpk::error_t									findLastSlash			(const ::gpk::view_const_char & path);
