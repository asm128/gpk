...
namespace gpk
{
	...
	template <typename _t, size_t _arraySize>						stincxp	uint32_t	byte_count	(const _t (&/*staticArray*/)[_arraySize])				noexcept	{ return sizeof(_t) * _arraySize;				}
	template <typename _t, size_t _sizeDepth, size_t _sizeWidth>	stincxp	uint32_t	byte_count	(const _t (&/*staticArray*/)[_sizeDepth][_sizeWidth])	noexcept	{ return sizeof(_t) * _sizeDepth*_sizeWidth;	}
}
