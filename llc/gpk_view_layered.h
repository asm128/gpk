...
#pragma pack(push, 1)
	...
	public:
		...
		inline															view_layered				(_tElement* dataElements, uint32_t layerWidth, uint32_t layerHeight, uint32_t layerCount)	: Data{dataElements}, Size{layerWidth, layerHeight, layerCount}, LayerSize(layerWidth * layerHeight) {
			gthrow_if(0 == dataElements && 0 != Size.x && 0 != Size.y && 0 != Size.z, "Invalid parameters. Address: %p. Metrics: {%u, %u, %u}", dataElements, Size.x, Size.y, Size.z);	// Crash if we received invalid parameters in order to prevent further malfunctioning.
		}
		...
		// Operators
							::gpk::view_grid<_tElement>					operator[]					(uint32_t layer)																			{ gthrow_if(0 == Data, "%s", "Uninitialized array pointer."); gthrow_if(layer >= Size.z, "Invalid layer: %i.", layer); return ::gpk::view_grid<_tElement		>(&Data[layer * LayerSize], Size.x, Size.y); }
							::gpk::view_grid<const _tElement>			operator[]					(uint32_t layer)														const				{ gthrow_if(0 == Data, "%s", "Uninitialized array pointer."); gthrow_if(layer >= Size.z, "Invalid layer: %i.", layer); return ::gpk::view_grid<const _tElement>(&Data[layer * LayerSize], Size.x, Size.y); }
