...
#pragma pack(pop)
					...
					uint32_t												update_crc						(const ::gpk::array_view<const ubyte_t>& buf, uint32_t crc)		;
	static inline	uint32_t												get_crc							(const ::gpk::array_view<const ubyte_t>& buf)					{ return update_crc(buf, 0xffffffffL) ^ 0xffffffffL; }
