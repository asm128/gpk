...
#pragma pack(push, 1)
	...
	struct SIPv4Endpoint {
		...
		inline			bool											operator!=			(const SIPv4Endpoint& other)	const	noexcept	{ return !operator==(other); }
						bool											operator==			(const SIPv4Endpoint& other)	const	noexcept	{
			return IP[0]	== other.IP[0]
				&& IP[1]	== other.IP[1]
				&& IP[2]	== other.IP[2]
				&& IP[3]	== other.IP[3]
...
#define GPK_IPV4_EXPAND_PTR_IP(addr) &addr.IP[0]	\
	...
	stainli	::gpk::error_t			tcpipAddress		(const ::gpk::vcc & hostName, uint16_t portRequested, uint32_t adapterIndex, TRANSPORT_PROTOCOL mode, SIPv4Endpoint & address)	{
		address.Port													= portRequested;
		return ::gpk::tcpipAddress(hostName, portRequested, adapterIndex, mode, GPK_IPV4_EXPAND_PTR_IP(address), &address.Port);
	}
