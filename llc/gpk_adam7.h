...
namespace gpk
{
					::gpk::error_t							adam7Sizes						(::gpk::view<::gpk::n2<uint32_t>> & imageSizes, const ::gpk::n2<uint32_t> & imageSize);
	...
	static			::gpk::error_t							adam7Interlace					(::gpk::view<::gpk::img<_tTexel>> images, ::gpk::view2d<_tTexel> & out_View)				{
		for(uint32_t iImage = 0; iImage < images.size(); ++iImage) {
			::gpk::n2<uint32_t>									offsetMultiplier				= {1, 1};
			::gpk::n2<uint32_t>									offsetBase						= {0, 0};
			...
			for(uint32_t x = 0; x < image.metrics().x; ++x) {
				::gpk::n2<uint32_t>									targetCell						= {x * offsetMultiplier.x + offsetBase.x, y * offsetMultiplier.y + offsetBase.y};
