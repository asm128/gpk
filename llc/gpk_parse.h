...
namespace gpk
{
	...
	template<typename _tInt>	inline	::gpk::error_t			parseIntegerDecimal				(const ::gpk::view_const_string& sourceChars, _tInt* number_)	{ uint64_t number; const ::gpk::error_t countDigits = ::gpk::parseArbitraryBaseInteger(10, "0123456789"			, sourceChars, &number); gpk_necall(countDigits, "%s", "Unknown error."); *number_ = (_tInt)number; return countDigits; };
	template<typename _tInt>	inline	::gpk::error_t			parseIntegerHexadecimal			(const ::gpk::view_const_string& sourceChars, _tInt* number_)	{ uint64_t number; const ::gpk::error_t countDigits = ::gpk::parseArbitraryBaseInteger(16, "0123456789abcdef"	, sourceChars, &number); gpk_necall(countDigits, "%s", "Unknown error."); *number_ = (_tInt)number; return countDigits; };
