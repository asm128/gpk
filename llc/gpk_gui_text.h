...
namespace gpk
{
	...
	static	::gpk::error_t	textLineDrawFixedSize			(::gpk::view2d<_tColor> & target, const ::gpk::view2d<_tColor> & viewTextureFont, uint16_t characterCellsX, int16_t dstOffsetY, const ::gpk::n2u8 & sizeCharCell, const ::gpk::vcs & text0, const ::gpk::n2i16 dstTextOffset)	{	// --- This function will draw some coloured symbols in each cell of the ASCII screen.
		for(int32_t iChar = 0, charCount = (int32_t)text0.size(); iChar < charCount; ++iChar) {
			...
			const ::gpk::n2i16			coordCharTable					= {coordTableX * sizeCharCell.x, coordTableY * sizeCharCell.y};
	...
	static	::gpk::error_t	textLineDrawFixedSizeLit		(::gpk::view2d<_tColor> & target, const ::gpk::view_bit<uint64_t> & viewTextureFont, const ::gpk::n2u16 & viewMetrics, uint16_t characterCellsX, int16_t dstOffsetY, const ::gpk::n2u8 & sizeCharCell, const ::gpk::vcs & text0, const ::gpk::n2i16 dstTextOffset, const _tColor& color)	{	// --- This function will draw some coloured symbols in each cell of the ASCII screen.
		...
		for(int32_t iChar = 0, charCount = (int32_t)text0.size(); iChar < charCount; ++iChar) {
			...
			gerror_if(errored(::gpk::grid_raster_alpha_bit(target.metrics().Cast<uint16_t>(), viewTextureFont, viewMetrics, ::gpk::rect2i16{dstTextOffset + dstRect0.Offset, sizeCharCell.Cast<int16_t>()}, srcRect0.Offset, dstCoords)), "%s", "I believe this never fails.");
	...
	static	::gpk::error_t	textLineDrawAlignedFixedSize	(::gpk::view2d<_tColor> & targetView, const ::gpk::view2d<_tColor> & fontAtlas, uint16_t lineOffset, const ::gpk::n2u16 & targetSize, const ::gpk::n2u8 & sizeCharCell, const ::gpk::vcs & text0)	{	// --- This function will draw some coloured symbols in each cell of the ASCII screen.
		const ::gpk::n2i16			dstTextOffset					= {(int16_t)targetSize.x / 2 - (int16_t)::gpk::textLineCalcWidth(sizeCharCell, text0) / 2, };
	...
	template<typename _tColor>
	static	::gpk::error_t	textLineDrawAlignedFixedSizeLit	(::gpk::view2d<_tColor> & targetView, const ::gpk::view_bit<uint64_t> & fontAtlas, const ::gpk::n2u16 & viewMetrics, uint16_t lineOffset, const ::gpk::n2u16 & targetSize, const ::gpk::n2u8 & sizeCharCell, const ::gpk::vcs & text0, const _tColor & color)	{	// --- This function will draw some coloured symbols in each cell of the ASCII screen.
		const ::gpk::n2i16			dstTextOffset					= {(int16_t)targetSize.x / 2 - (int16_t)::gpk::textLineCalcWidth(sizeCharCell, text0) / 2, };
		uint16_t					dstOffsetY						= int16_t(lineOffset * sizeCharCell.y);
	...
	template<typename _tColor>
	static	::gpk::error_t	textLineRaster					(const ::gpk::n2u16 & targetMetrics, const ::gpk::n2u8 & fontCharSize, const ::gpk::rect2<int16_t> & targetRect, const ::gpk::SImageMonochrome<_tColor> & fontTexture, const ::gpk::vcs & textToRaster, ::gpk::apod<::gpk::n2u16> & dstCoords)		{
		const uint16_t				charsPerRow						= uint16_t(fontTexture.Pitch / fontCharSize.x);
		for(uint32_t iChar = 0, countChars = textToRaster.size(); iChar < countChars; ++iChar) {
			...
			const int16_t				coordTableX						= charToDraw % charsPerRow;
			const int16_t				coordTableY						= charToDraw / charsPerRow;
			const ::gpk::n2i16			coordCharTable					= {int16_t(coordTableX * fontCharSize.x), coordTableY * fontCharSize.y};
			const ::gpk::rect2i16		srcRect0						= {coordCharTable, fontCharSize.Cast<int16_t>()};
			::gpk::rect2i16				dstRect1						= {{int16_t(targetRect.Offset.x + fontCharSize.x * iChar), targetRect.Offset.y}, fontCharSize.Cast<int16_t>()};
			//dstRect1.Size.x		+= ::gpk::min(0, (int32_t)targetRect.Offset.x);
			dstRect1.Size.y			+= int16_t(::gpk::min<int16_t>(0, targetRect.Offset.y));
