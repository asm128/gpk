...
#	include <Windows.h>
	...
	for(uint32_t iFile		= 0; iFile		< input.Files	.size(); ++iFile) 
		gpk_necall(output.push_back(input.Files[iFile]), "Out of memory?");
	for(uint32_t iFolder	= 0; iFolder	< input.Folders	.size(); ++iFolder) 
		gpk_necall(::gpk::pathList(input.Folders[iFolder], output), "Unknown error!");
	return 0;
}
	...
	do if(	0 != strcmp(fdFile.cFileName,  ".")
		...
		gpk_necall(output.push_back({sPath, (uint32_t)lenPath}), "Failed to push path to output list");
	}
	...
	do if(	0 != strcmp(fdFile.cFileName,  ".")	
		...
		if(fdFile.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
			...
			gpk_necall(newFolderIndex, "Out of memory?");
			gpk_necall(::gpk::pathList(sPath, pathContents.Folders[newFolderIndex]), "Unkown error!");
			info_printf("Directory: %s.", sPath);
		}
		else {
			gpk_necall(pathContents.Files.push_back({sPath, (uint32_t)lenPath}), "Failed to push path to output list");
