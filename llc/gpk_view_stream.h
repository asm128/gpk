...
namespace gpk
{
	...
	struct view_stream : public view<_tElement> {
		...
		template<typename _tPOD, size_t _sCount>	inline	::gpk::error_t					read_pod							(_tPOD (&read)[_sCount])				{ stacxpr const int32_t	sizeToCopy = (int32_t)(sizeof(_tPOD) * _sCount);	memcpy(read, &Data[CursorPosition], sizeToCopy);		CursorPosition += sizeToCopy;		return _sCount; }
		...
		template<typename _tPOD, size_t _sCount>	inline	::gpk::error_t					write_pod							(const _tPOD (&towrite)[_sCount])		{ stacxpr const int32_t	sizeToCopy = (int32_t)(sizeof(_tPOD) * _sCount);	memcpy(&Data[CursorPosition], towrite, sizeToCopy);	CursorPosition += sizeToCopy;		return sizeToCopy; }
