...
namespace gpk
{
	...
	struct enum_definition {
		...
		inline													enum_definition						()																		: Name("Enum definition name not set.")						{}
		inline static		enum_definition<_tValue>&			get										()																		{
			static	enum_definition<_tValue>							valueRegistry;
			return valueRegistry;
		}
		inline static		_tValue								init									(const ::gpk::label& enumName)										{
		...
		inline				::gpk::error_t						get_value								(const ::gpk::view_const_string& name, _tValue& value)	const			{ return get_value(name.begin(), value); }
			...
			return -1;
							...
							_tValue								get_value								(const ::gpk::label& name)								const			{
			for(uint32_t i=0, count = Names.size(); i<count; ++i)
				if(name == Names[i])
					return Values[i];
			...
			return Values[index];
							...
							::gpk::error_t						get_label_by_index						(uint32_t index, ::gpk::label& value)					const			{
			if( index < Names.size() ) {
				value													= Names[index];
				return 0;
			}
			...
			return -1;
							...
							::gpk::label						get_label_by_index						(uint32_t index)										const			{
			if( index < Names.size() ) {
				return Names[index];
			}
			else {
				...
				return ::gpk::UNDEFINED_ENUM_VALUE_STR;
							...
							::gpk::error_t						get_value_index							(const ::gpk::label& name, int32_t& index)				const			{
			for(uint32_t i=0, count = Names.size(); i < count; ++i)
				if(name == Names[i]) {
					index													= (int32_t)i;
					return 0;
			...
			return index											= -1;
							...
							int32_t								get_value_index							(const ::gpk::label& name)								const			{
			for(uint32_t i=0, count = Names.size(); i < count; ++i)
				if(name == Names[i])
					return (int32_t)i;
			...
			return -1;
							...
							::gpk::error_t						get_value_label							(const _tValue& value, ::gpk::label& name)				const			{
			...
			return ::gpk::UNRESOLVED_ENUM_LABEL_STR;
							...
							::gpk::error_t						add_value								(const _tValue& value, const ::gpk::label& name)						{
			for(uint32_t i=0, count = Values.size(); i < count; ++i)
				if(Values[i] == value) {
