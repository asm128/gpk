...
namespace gpk
{
	...
	static inline void*																			gpk_malloc					(size_t size)													{ return _aligned_malloc(size, GPK_MALLOC_ALIGN);	}
	static inline void																			gpk_free					(void* ptr)														{ _aligned_free(ptr);										}
#elif defined(GPK_LINUX) || defined(GPK_ANDROID)
	static inline void*																			gpk_malloc					(size_t size)													{ return ::memalign(GPK_MALLOC_ALIGN, size);			}
	static inline void																			gpk_free					(void* ptr)														{ ::free(ptr);												}
#endif
