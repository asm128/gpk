...
#pragma pack(push, 1)
	template <typename... _tArgs>	struct member_registry {
		...
		static inline			const ::gpk::view_array<const ::gpk::DATA_TYPE			>	get_data_type_ids				()							noexcept	{ static const ::gpk::DATA_TYPE			results[] = {			_tArgs::get_member_data_type_id	()					...,					::gpk::DATA_TYPE_UNKNOWN	}; return results; }
		static inline			const ::gpk::view_array<const uint32_t					>&	get_member_sizes				()							noexcept	{ static const uint32_t					results[] = {(uint32_t)	_tArgs::get_member_size			()					...,											 0	}; static const ::gpk::view_array<const uint32_t				> results_view = {results}; return results_view;	}
	}; // struct
