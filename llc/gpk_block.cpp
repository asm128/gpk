...
::gpk::error_t			gpk::blockRecordPath		(::gpk::apod<sc_t> & fileName, const ::gpk::SRecordMap & indices, ::gpk::vcsc_c & dbName, ::gpk::vcsc_c & dbPath)	{
	::gpk::apod<sc_t>			finalPath					= dbPath;
	gpk_necs(gpk::blockFilePath(finalPath, dbName, dbPath));
	gpk_necs(gpk::blockFileName(fileName, indices.IdBlock, dbName, finalPath));
	return 0;
}
::gpk::error_t			gpk::blockFilePath			(::gpk::apod<sc_t> & finalPath, ::gpk::vcsc_c & dbName, ::gpk::vcsc_c & dbPath) {
	finalPath				= dbPath;
	if(finalPath.size())
...
::gpk::error_t			gpk::blockFileName			(::gpk::apod<sc_t> & fileName, const uint32_t idBlock, ::gpk::vcsc_c & dbName, ::gpk::vcsc_c & folderName)	{
	if(folderName.size()) {
...
::gpk::error_t			gpk::blockConfigLoad		(::gpk::SBlockConfig & loaded, const ::gpk::SJSONReader & reader, int32_t iNode, const ::gpk::SBlockConfig & configDefault)	{
	...
	::gpk::error_t				indexJSONBlockSize			= ::gpk::jsonExpressionResolve(::gpk::vcst_t{"blocksize"}, reader, (uint32_t)iNode);
	::gpk::error_t				indexJSONDeflate			= ::gpk::jsonExpressionResolve(::gpk::vcst_t{"deflate"  }, reader, (uint32_t)iNode);
